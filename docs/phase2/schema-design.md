...
| lodging_scene | pct(lodging_5km) | **osm** | 確定案 |
...
充実度ゲート = 1 if（dining_5km と lodging_5km がともに下位25%を上回り
                （絶対数・人口あたりのどちらか高い方のパーセンタイルで判定）、
                かつ culture_5km または scenic_nature が下位25%を上回る）else 0
```

飲食・宿泊を**絶対数と人口あたりのunion**で判定するのは、ツェルマット・ブレッド・サンジミニャーノのような
小規模な有名観光地が絶対数のみだと軒並みゲート落ちするため（人口あたりなら上位に来る）。
逆に**人口あたりへの全面置換は不採用**：検証したところロンドン（絶対数P99→人口あたりP2）・ウィーン・ローマなど
大都市34件が新たにゲート落ちする逆バイアスが出る。unionなら新たに落ちる地点はゼロで、小規模観光地20件が救済される。
