...
def main():
    ...
    lines += [
        ...
        "3. **POIはFoursquare OS Places由来**（2026-07-09版・閉店除外済み）。OSMは自然地物・展望地点・"
        "歩行者エリア（osm_features）のみで、同ブロックには登録密度の地域差（小さな町で過少）が残る。"
        "Foursquareの月次更新への追随は再取得経路の確認が前提（調査レポート§4）。",
        ...
        "祝日は全国区＋その地点の州・地域の祝日（Wikidata行政区分チェーンでISO 3166-2に紐付けて判定）。"
        "物価水準（PLI）は年次・EU27=100の相対値で変動は小さく年1回の更新で十分。"
        "日照時間は各月15日時点の天文計算値。",
        "",
        "## 5. 判定",
        "",
